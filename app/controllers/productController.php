<?php
	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
			...
			$ubicacion = $this->limpiarCadena($_POST['producto_ubicacion']);


		    # Verificando campos obligatorios #
            if($codigo=="" || $nombre=="" || $precio_compra=="" || $precio_venta=="" || $stock==""|| $stock==""){
            	$alerta=[
					"tipo"=>"simple",
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,$#\-\/ ]{1,800}",$nombre)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
			...
			if ($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,100}", $ubicacion)) {
                    $alerta = [
                        "tipo" => "simple",
                        "titulo" => "Ocurrió un error inesperado",
    		...
    		$producto_datos_reg=[
				...
				[
					"campo_nombre"=>"producto_ubicacion",
    ...
    $campos = "producto.producto_id, producto.producto_codigo, producto.producto_nombre, producto.producto_stock_total, 
               producto.producto_precio_venta, producto.producto_foto, producto.producto_ubicacion, categoria.categoria_nombre";
    ...
    if (!empty($busqueda)) {
        $consulta_datos = "SELECT $campos FROM producto 
                           ...
                           OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%' 
                           ORDER BY producto_nombre ASC LIMIT $inicio, $registros";
    ...
    if ($total >= 1 && $pagina <= $numeroPaginas) {
        ...
        foreach ($datos as $rows) {
            ...
            $tabla .= '<figure class="media-left">';
            $tabla .= '<p class="image is-64x64">';
            ...
            $tabla .= '</p>';
            $tabla .= '</figure>';
            $tabla .= '<div class="media-content">';
            $tabla .= '<div class="content">';
            $tabla .= '<p>';
            $tabla .= '<strong>' . $contador . ' - ' . $rows['producto_nombre'] . '</strong><br>';
            $tabla .= '<strong>CODIGO:</strong> ' . $rows['producto_codigo'] . ', ';
            $tabla .= '<strong>PRECIO:</strong> $' . $rows['producto_precio_venta'] . ', ';
            $tabla .= '<strong>STOCK:</strong> ' . $rows['producto_stock_total'] . ', ';
            $tabla .= '<strong>UBICACIÓN:</strong> ' . $rows['producto_ubicacion'] . ', ';
            $tabla .= '<strong>CATEGORIA:</strong> ' . $rows['categoria_nombre'];
            $tabla .= '</p>';
            $tabla .= '</div>';
            $tabla .= '<div class="has-text-right">';
            $tabla .= '<a href="' . APP_URL . 'productPhoto/' . $rows['producto_id'] . '/" class="button is-info is-rounded is-small">';
            $tabla .= '<i class="far fa-image fa-fw"></i></a>';
            $tabla .= '<a href="' . APP_URL . 'productUpdate/' . $rows['producto_id'] . '/" class="button is-success is-rounded is-small">';
            $tabla .= '<i class="fas fa-sync fa-fw"></i></a>';
            $tabla .= '<form class="FormularioAjax is-inline-block" action="' . APP_URL . 'app/ajax/productoAjax.php" method="POST">';
            $tabla .= '<input type="hidden" name="modulo_producto" value="eliminar">';
            $tabla .= '<input type="hidden" name="producto_id" value="' . $rows['producto_id'] . '">';
            $tabla .= '<button type="submit" class="button is-danger is-rounded is-small">';
            $tabla .= '<i class="far fa-trash-alt fa-fw"></i></button></form></div></div></article><hr>';
        ...
        $tabla .= '<p class="has-text-centered pb-6"><i class="far fa-grin-beam-sweat fa-5x"></i></p>';
        $tabla .= '<p class="has-text-centered">No hay productos registrados en esta categoría</p>';
    }
    ...
    return $tabla;
		...
		public function actualizarProductoControlador(){
		    ...
		    if($marca!=""){
			   ...
			   if($ubicacion!=""){
		    	if($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,30}",$ubicacion)){
			    	$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"La MARCA no coincide con el formato solicitado",
